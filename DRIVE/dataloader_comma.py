...
class CommaDataset(Dataset):
    def __init__(
        ...
        #select noise added dataset
        if self.img_noise:
            print("using img_noise")
            data_path = f"{dataset_path}/comma_{dataset_type}_w_desired_filtered_{img_noise}.h5py"
            print(f"current dataset: {data_path}")
        self.people_seqs = []
        self.h5_file = h5py.File(data_path, "r")
        corrupt_idx = 62
        print("data_path-----",data_path,"-----------------")#debug
        self.keys = list(self.h5_file.keys())
        # self.keys = list(self.h5_file.keys())[:5]

        if dataset_type == "train":
            self.keys.pop(corrupt_idx)
        # self.keys = self.keys[:5]#debug
    def __len__(self):
