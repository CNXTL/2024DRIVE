...
class TVTN(nn.Module):
    ...
    def __init__(self, multitask="angle", backbone="resnet", device="cuda:0", multitask_param=True, concept_features=False, train_concepts=False, return_concepts=False):
        super(TVTN, self).__init__()
        self.device = device
    ...
    def forward(self, img, angle, distance, vego):
        ...
        # print("cls_tokens.grad",cls_tokens.grad)
        cls_tokens = self.cls_token.expand(B, -1, -1).cuda()  # stole cls_tokens impl from Phil Wang, thanks
        # print("cls_tokens.grad",cls_tokens.grad)
        # cls_tokens.requires_grad=True
        # print("cls_token grad:",cls_tokens.requires_grad)
        ...
        if self.multitask != "multitask":
            res = x[:,1:F+1,:], attentions,probs #probs match the predict step

            return res
            # return res, probs # we want to exclude the starting token since we don't have any previous knowledge about it       
        
        else:
            res = (x[:,1:F+1,:], x2[:,1:F+1,:],self.multitask_param_angle, self.multitask_param_dist), attentions,probs
            # return res, probs # we want to exclude the starting token since we don't have any previous knowledge about it 
            return res
